		...
		<div class="dialog">
			<el-dialog title="编辑场景" :visible.sync="dialogFormUpdateVisible">
				<el-form :model="prepareCard" status-icon :rules="check" ref="upform" label-width="100px" class="demo-ruleForm">
				  <el-form-item label="场景名称" prop="sceneName">
				    <el-input v-model="prepareCard.sceneName" autocomplete="off"></el-input>
				  </el-form-item>
				  <el-form-item label="场景描述" prop="description">
				    <el-input type="textarea" v-model="prepareCard.description"></el-input>
				  </el-form-item>
				  <el-form-item>
...
<script>
    ...
    methods: {
        async switchChange(sceneId, switchTag) {
            var data = {};
        ...
        toContent(sceneId) {
            // uni.navigateTo({
            // 	url:'./routeContent?sceneId='+sceneId+'&projectType='+this.projectType
            // })
            // this.$router.push({name:'routeContent',query:{sceneId:sceneId,projectType:this.projectType}})
            this.$router.push(`/webgis/routeContent?sceneId=${sceneId}&projectType=${this.projectType}`);
        },
		handleRemove(file) {
			this.fileList=[];
			this.form.img=""
        ...
        async delScene(sceneId) {
            var req = {};
            req.sceneId = sceneId;
            const res = await api.delScene(req);
            if (res.code == 200) {
