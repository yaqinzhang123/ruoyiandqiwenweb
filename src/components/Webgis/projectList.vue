<template>
	<div>
		<div class="optionBar">
			<el-row>
			  ...
			  <el-col :span="12" class="btnstyle">
				  <el-button type="primary" @click="dialogFormVisible = true" >新增路由规则</el-button>
			  </el-col>
			</el-row>
		</div>
		<div class="list" >
				  ...
				  <el-col v-for="(item,index) in routeCards" :key="index" :span="6">
					  <div  class="" >
					  	<el-card class="content" shadow="hover">
							<div v-if="item.switchTag" class="status-open"></div>
							<div v-if="!item.switchTag" class="status-close"></div>
					  		<div class="project" @click="toContent(item.projectId)">
					  		<div class="project-name">
					  			<i class='el-icon-coin'></i>
					  			<span>服务名称：{{item.projectName}}</span>
					  		</div>
					  		<div class="projectDesc">
					  			服务端口：<span>{{item.projectPort}}</span>
					  		</div>
					  		<!-- <div>
					  			<h3 class='title'>路由：</h3><span>{{route}}</span>
					  		</div> -->
					  		<div class="projectDesc">
					  			创建时间：<span>{{item.createTime}}</span>
					  		</div>
					  		<div class="projectDesc">
					  			服务备注：<span>{{item.projectDesc}}</span>
					  		</div>
					  		</div>
					  			...
					  			<div>
					  				<el-row>
					  					<el-col :span="8" class="switchServer">
					  						<span class="space">关</span>
					  						<el-switch @change="switchChange(item.projectId,item.switchTag)" v-model="item.switchTag"></el-switch>
					  						<span class="space">开</span>
		...
		<div class="dialog">
			<el-dialog title="新建资源服务" :visible.sync="dialogFormVisible">
				<el-form :model="form" status-icon :rules="check" ref="form" label-width="100px" class="demo-ruleForm">
				  ...
				  <el-form-item label="项目端口" prop="projectPort">
				    <el-input v-model.number="form.projectPort"></el-input>
				  </el-form-item>
		...
		<div class="dialog">
			<el-dialog title="编辑资源服务" :visible.sync="dialogFormUpdateVisible">
				<el-form :model="prepareCard" status-icon :rules="check" ref="upform" label-width="100px" class="demo-ruleForm">
				  ...
				  <el-form-item label="项目备注" prop="projectDesc">
				    <el-input type="textarea" v-model="prepareCard.projectDesc"></el-input>
				  </el-form-item>
				  <el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async initList(){
				this.listData.projectType=this.projectType
